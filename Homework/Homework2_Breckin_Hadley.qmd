...
**str_length = counts how many characters are in the title**
**str_sub = It would exact the first four characters of the album release date which would be the year**
**str_lower = This would just make all of the titles all lower case.**
**str_replaceall = This would replace all of the "-" with "/"**
...
```{r}
filter(spot_smaller, str_detect(spot_smaller$subgenre, "pop"))
```
...
spot_smaller|>
  mutate(
    sub_pop = str_detect(spot_smaller$subgenre, "pop" ),
...
spot_smaller|>
  mutate( title_length = str_length(title),
          sub_pop = iflese(str_detect(spot_smaller$subgenre, "pop"," Genre with pop" = "TRUE", "Genre without pop" = "FALSE"))|>
  group_by(sub_pop)|>
  summarise(mean_title_length = mean(title_length))
```
...
bigspotify|>
  ...
  !str_detect(track_name, "[^//$].*(feat.|with).*\\$.*"))|>
  select(track_name)|>
print(n=25)
...
- "^.*$" <- This would give an output where it starts and ends with anything repeated 0 or more times  
- "\\{.+\\}" <- This would give and output with anything containing {} brackets

```{r}
#^.*$
str_view(spot_smaller$album_name, "^.*$")
...
```{r}
spot_smaller |>
  mutate(numb_word = str_count(title, "\\b[^ ]+\\b"))
```
...
- (.)\1\1 -> 3 of the same charcter in a row
- "(.)(.)(.).*\\3\\2\\1" -> 
...
```{r}
spot_smaller |>
  mutate( New_release_date = str_replace(album_release_date,"(....)-(..)-(..)", "\\2 - \\3 - \\1")) |>
  select(album_name,New_release_date, album_release_date)
```
...
sentences |> 
  ...
  head(5)
```
>This code will switch the order of the second and third words. 
...
>This expression (.) matches any single character. And this expression (.) also mathces any single character \2 → backreference to group 2, so the same character as B must appear next.
\1 → backreference to group 1, so the same character as A must appear after that.

>Examples abba, noon, 1221
...
as_tibble(words) |>
  ...
  arrange(desc(vowels)) |>
  slice_max(vowels, with_ties = TRUE)
```
...
sentences |>
  ...
  mutate(
    first_word = str_extract(sentence, "^\\w+"),
    first_ed   = str_extract(sentence, "\\b\\w+ed\\b")
  )
...
sentences |>
  as_tibble() |>
  rename(sentence = value) |>
  mutate(contraction = str_extract(sentence, "\\b\\w+'\\w+\\b")) |>
...
This swaps the first and last letters of the then prints every words that are the result of swapping the first/last letters of some word (including words unchanged by the swap).

>Line 1: In temp we string replace with the words data set. We want to replace any word that starts with A-Z or a-z then replace it with the last letter of the word which ranges from a-z

>Line 2: Create a tibble of the words data set

>Line 3: Only print the x values

>Line 4: Prints all rows instead of truncating the tibble.
