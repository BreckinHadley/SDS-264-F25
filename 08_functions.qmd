...
se(0:5)
se(0:5, na = TRUE)
vector4 <- c(1,2,3,4,5,NA)
se(vector4)
se(vector4,na.rm = TRUE)
...
flights|>
  ...
  summarize(
    ...
    mean_dep_dealy = mean(dep_delay, na.rm = TRUE),
...
flights|>
  summarize(arr_NA = sum(is.na(arr_delay)),
            ...
            both_NA = both(arr_delay,dep_delay))
...
#Differnce in time in minutes 
time_diff_min <- function(time1, time2) {
  hour1 <- time1 %/% 100
  min1 <- time1 %% 100
  hour2 <- time2 %/% 100
  min2 <- time2 %% 100
...
#Area CM -> Inches
Area_cm_to_inch <- function(lengthcm, widthcm) {
...
This returns the number of entries where the previous number is greater then the current number.
...
```{r}
summary6 <- function(data, var, na_option = TRUE) {
  data |> summarize(
...
```{r}
two_var_summary6 <- function(data, var, var2, na_option = TRUE) {
  data |> 
    group_by(pick({{ var2 }})) |> 
    summarize(
    mean = mean({{ var }}, na.rm = na_option),
...
two_var_summary6(mpg,hwy,drv)
two_var_summary6(gss_cat,age,partyid)
```
...
scatterplot <- function(df, varX, varY) {
  label <- rlang::englue("A scatterplot of {{var}}")
...
scatterplot(two_var_summary6, mean, sd)
