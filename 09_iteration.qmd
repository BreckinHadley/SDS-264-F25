...
This code will use a while loop to flip a coin. The function will count how many flips it takes to get 3 heads in a row.
...
null_diffs <- vector("double", 1000)
  for( i in 1:1000){
    dolphin_null <- tibble(treatment = sample(dolphin_data$treatment),
                       improve = c(rep("Yes", 10), rep("No", 5), 
                                   rep("Yes", 3), rep("No", 12)))
    null_sum <- dolphin_null|>
      group_by(treatment) |>
      summarize(prop_yes = mean(improve == "Yes"))
      null_diffs[i] <- null_sum[[2]][2] - null_sum[[2]][1]
  }
null_diffs


# Step 3
null_world <- tibble(null_diffs = null_diffs)
ggplot(null_world, aes(x = null_diffs)) +
  geom_histogram() +
  geom_vline(xintercept = observed_diff, color = "red")

p_value <- sum(abs(null_diffs) >= abs(observed_diff)) / 1000
p_value
```
...
#For Loop
mtcars
means1 <- vector("double", length(mtcars))
...
#Across Function
summarize(mtcars, across(everything(), mean, .names = "mean_{.col}"))
...
#For loop
num_unique <- vector("double", length(iris))
for (i in 1:ncol(iris)) {
  num_unique[i] = n_distinct(iris[[i]])
}
num_unique
```
...
```{r}
#Creates a function called show_missing 
show_missing <- function(df, group_vars, summary_vars = everything()) { 
  df |> 
    group_by(pick({{ group_vars }})) |> #groups by given grouping variables (use pick because we want a series of variables for grouping)
    summarize(
      #count the number of NAs in each of the summary variables and ungroups
      across({{ summary_vars }}, \(x) sum(is.na(x))),
      .groups = "drop"
    ) |>
    select(where(\(x) any(x > 0))) #select only columns that have at least one NA
}
#Outputs how many flights grouped bu year, month, and day that have a missing value in each column
...
summary_stats<- function(df,vars,stat_fcts) {
  df |>
    summarize(across(.cols = {{vars}}, .fn = {{stat_fcts}}), n=n())
}
